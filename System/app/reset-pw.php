...
try {
    ...
    if ($rec == '0') {
     ...
     No account is associated with email <strong>' .
            $opt .
            '</strong>
	 </div>
     ';
        ...
        foreach ($result as $row) {
            ...
            $stmt = $conn->prepare(
                'DELETE FROM tbl_tokens WHERE email = :email'
            );
            $stmt->bindParam(':email', $opt);
            $stmt->execute();

            $stmt = $conn->prepare(
                'INSERT INTO tbl_tokens (email, token) VALUES (:email, :token)'
            );
            $stmt->bindParam(':email', $opt);
...
Đã xảy ra lỗi vui lòng liên hệ với chúng tôi để được trợ giúp thêm.
     ...
     Một liên kết để đặt lại mật khẩu của bạn đã được gửi đến' .
                    $mymail .
                    '.
	 </div>
     ';
