...
<body class="not-transparent-header">
	...
	<div class="container-wrapper">
       ...
       try {
           ...
           $stmt = $conn->prepare(
               "SELECT * FROM tbl_users WHERE role = 'employer' ORDER BY first_name LIMIT $page1,16"
           );
           $stmt->execute();
       ...
       try {
           ...
           $stmt = $conn->prepare(
               "SELECT * FROM tbl_users WHERE role = 'employer' ORDER BY first_name"
           );
           $stmt->execute();
           $result = $stmt->fetchAll();
...
} ?>  href="employers.php?page=<?php echo "$b"; ?>"><?php echo $b .
    ' '; ?></a></li><?php }
            print '<li class="paging-nav"';
            if ($page == $records) {
                print 'class="disabled"';
