	...
	<header id="header">
		...
		<div class="main-wrapper">
         ...
         try {
             ...
             $stmt = $conn->prepare(
                 "SELECT * FROM tbl_other_attachments WHERE member_no = '$myid' ORDER BY id LIMIT $page1,5"
             );
             $stmt->execute();
             ...
             'id'
									...
									<a href="app/drop-attachment.php?id=<?php echo $row[
             'id'
         ]; ?>" onclick = "return confirm('Are you sure you want to delete this attachment ?')" class="btn btn-primary btn-sm btn-inverse">Delete</a>
             ...
             'id'
							        ...
							        <div class="form-group"> 
								    <label>Chọn tệp đính kèm <i>Để trống nếu bạn không muốn cập nhật</i></label>
        ...
        try {
            ...
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare(
                "SELECT * FROM tbl_other_attachments WHERE member_no = '$myid' ORDER BY id"
            );
            $stmt->execute();
            $result = $stmt->fetchAll();
            foreach ($result as $row) {
...
} ?> href="attachments.php?page=<?php echo "$b"; ?>"><?php echo $b .
    ' '; ?></a></li><?php }
            print '<li class="paging-nav"';
            if ($page == $records) {
                print 'class="disabled"';
