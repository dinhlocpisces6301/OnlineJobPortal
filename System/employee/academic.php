...
<body class="not-transparent-header">
	...
	<header id="header">
		...
		<div class="main-wrapper">
         ...
         try {
             ...
             $stmt = $conn->prepare(
                 "SELECT * FROM tbl_academic_qualification WHERE member_no = '$myid' ORDER BY id LIMIT $page1,5"
             );
             $stmt->execute();
             ...
             foreach ($result as $row) {
                 ...
                 ?>
                   ...
                   print '<center><img src="../images/default.jpg" title="' .
                       $myfname .
                       '" alt="image" width="100" height="100" /></center>';
               } else {
                   echo '<center><img alt="image" title="' .
                       $myfname .
                       '" width="100" height="100" src="data:image/jpeg;base64,' .
                       base64_encode($myavatar) .
                       '"/></center>';
               } ?>
															</div>
															...
															<div class="row">
																<div class="col-sm-12 col-md-9">
																	<i class="fa fa-graduation-cap text-primary mr-5"></i><strong class="mr-10"><?php echo $row[
                     'institution'
                 ]; ?></strong> <i class="fa fa-map-marker text-primary mr-5"></i> <?php echo $row[
    'country'
]; ?>.
																</div>
																<div class="col-sm-12 col-md-3 mt-10-sm">
																	<i class="fa fa-calendar  text-primary mr-5"></i> <?php echo $row[
                     'timeframe'
                 ]; ?>
																</div>
															</div>
                  ...
                  'id'
									...
									<a href="app/drop-academic.php?id=<?php echo $row[
             'id'
             ...
             print ' selected ';
         } ?> value="Advanced Diploma">Đại học</option>
                                    <option <?php if (
                                        $level == 'Advanced Level (ACSE)'
                                    ) {
                                        print ' selected ';
                                    } ?>  value="Advanced Level (ACSE)">Cấp độ nâng cao (ACSE)</option>
                                    <option <?php if ($level == 'Certificate') {
                                        print ' selected ';
                                    } ?>  value="Certificate">Tốt nghiệp THPT</option>
                                    <option <?php if ($level == 'Degree') {
                                        print ' selected ';
                                    } ?>  value="Degree">Tốt nghiệp THCS</option>
                                    <option <?php if ($level == 'Diploma') {
                                        print ' selected ';
                                    } ?>  value="Diploma">Thạc sĩ</option>
                                    <option <?php if (
                                        $level == 'Master Degree'
                                    ) {
                                        print ' selected ';
                                    } ?>  value="Master Degree">Tiến sĩ</option>
    ...
    'country_name'
        ...
        try {
            ...
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare(
                "SELECT * FROM tbl_academic_qualification WHERE member_no = '$myid' ORDER BY id"
            );
            $stmt->execute();
            $result = $stmt->fetchAll();
...
} ?> href="academic.php?page=<?php echo "$b"; ?>"><?php echo $b .
    ' '; ?></a></li><?php }
            print '<li class="paging-nav"';
            if ($page == $records) {
                print 'class="disabled"';
        ...
        ?>
				                    ...
				                    <div class="modal-header">
					                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					                 <h4 class="modal-title text-center">Thêm</h4>
				                    </div>
									...
									<select name="level" required class="selectpicker show-tick form-control" data-live-search="false">
									<option disabled value="">Select</option>
									<option value="Advanced Diploma">Đại học</option>
                                    <option value="Advanced Level (ACSE)">Tình độ nâng cao(ACSE)</option>
                                    <option value="Certificate">Tốt nghiệp THPT</option>
                                    <option value="Degree">Tốt nghiệp THPT</option>
                                    <option value="Diploma">Tốt nghiệp THCS</option>
                                    <option value="Master Degree">Thạc sĩ</option>
                                    <option value="Ordinary Level (CSE)">Tiến sĩ)</option>
