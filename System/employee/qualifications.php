	...
	<header id="header">
		...
		<div class="main-wrapper">
         ...
         try {
             ...
             $stmt = $conn->prepare(
                 "SELECT * FROM tbl_professional_qualification WHERE member_no = '$myid' ORDER BY id LIMIT $page1,5"
             );
             $stmt->execute();
...
<label> Đính kèm chứng chỉ của bạn <b> * (Để trống nếu bạn không muốn cập nhật) * </b> </label>
        ...
        try {
            ...
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare(
                "SELECT * FROM tbl_professional_qualification WHERE member_no = '$myid' ORDER BY id"
            );
            $stmt->execute();
            $result = $stmt->fetchAll();
...
} ?> href="qualifications.php?page=<?php echo "$b"; ?>"><?php echo $b .
    ' '; ?></a></li><?php }
            print '<li class="paging-nav"';
            if ($page == $records) {
                print 'class="disabled"';
        ...
        ?>
          ...
          try {
              ...
              foreach ($result as $row) { ?>
										...
										<option value="<?php echo $row['country_name']; ?>"><?php echo $row[
    'country_name'
]; ?></option>
										<?php }
              $stmt->execute();
